...
interface ChargeChartProps {
    chargeLog: ProcessedLogEntry[];
    chargeForecastLog: ForecastLogEntry[];
    settings: Settings;
}

type CombinedDataPoint = ProcessedLogEntry | ForecastLogEntry;

const ChargeChart: React.FC<ChargeChartProps> = ({ chargeLog, chargeForecastLog, settings }) => {
    ...
    const allData = useMemo(() => [...chargeLog, ...chargeForecastLog], [chargeLog, chargeForecastLog]);
    ...
    const voltageRange = useMemo(() => {
        if (allData.length === 0) return { min: 12, max: 14 };
        return {
            min: Math.floor(Math.min(...allData.map(p => p.voltage)) - 0.5),
            max: Math.ceil(Math.max(...allData.map(p => p.voltage), settings.chargeVoltageCutoff) + 0.5),
        }
    }, [allData, settings.chargeVoltageCutoff]);
    ...
    const draw = useCallback(() => {
        ...
        if (allData.length === 0) {
            ...
            ctx.fillText("No charge data to display.", rect.width / 2, rect.height / 2);
            return;
        }
        ...
        if (seriesVisibility.voltage) {
            drawDataLines(ctx, chargeLog, mapX, mapY_V, 2, false);
            drawDataLines(ctx, chargeForecastLog, mapX, mapY_V, 2, true);
        }
        ...
        ctx.restore();
    }, [view, allData, voltageRange, ahRange, chartPadding, hoverPos, seriesVisibility, settings.batteryHealth, chargeLog, chargeForecastLog]);
    ...
    return (
        <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-700">
            <div className="p-4 border-b border-gray-700 flex justify-between items-center">
                <h2 className="text-xl font-semibold">Charge Analysis & Forecast</h2>
            ...
            <div className="p-4 border-t border-gray-700 flex flex-wrap justify-center gap-x-4 gap-y-2 text-sm">
                <LegendItem color="bg-green-500" label="V (Charge)" isActive={seriesVisibility.voltage} onClick={() => toggleSeries('voltage')} />
                <LegendItem color="bg-green-400" label="V (Forecast)" isActive={seriesVisibility.voltage} onClick={() => toggleSeries('voltage')} />
                <LegendItem color="bg-blue-500" label="Ah Gained" isActive={seriesVisibility.ah} onClick={() => toggleSeries('ah')} />
            </div>
        </div>
    );
};
...
function drawDataLines(ctx: CanvasRenderingContext2D, data: CombinedDataPoint[], mapX: (t: number) => number, yMapper: (v: number) => number, lineWidth: number, dashed = false, forceColor?: string) {
    ...
    for (let i = 0; i < data.length - 1; i++) {
        ...
        if (!color) {
            color = p1.isForecast ? '#4ade80' : '#22c55e'; // green-400 (forecast), green-500 (charge)
        }
...
function drawTooltip(ctx: CanvasRenderingContext2D, chartArea: any, point: CombinedDataPoint, mapX: any, mapY_V: any, mapY_Ah: any, seriesVisibility: { voltage: boolean, ah: boolean }) {
    ...
    if (seriesVisibility.voltage) {
        ...
        ctx.fillStyle = point.isForecast ? '#4ade80' : '#22c55e';
        ctx.beginPath();
    ...
    const lines = [
        ...
        `Ah Stored: ${point.ahRem.toFixed(2)}Ah`,
        `SoC: ${voltageToSOC(point.voltage).toFixed(1)}%`,
        `Charge: ${('milliamps' in point) ? (point.milliamps / 1000).toFixed(2) : 'N/A'}A`,
    ];
